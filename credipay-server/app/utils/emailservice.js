...
var transporter_credipay = nodemailer.createTransport({
...
const emailsendBytitle = async (mapObj, template_name, lang, email)=>{
    ...
    try {
        ...
        const row_json = result;

        if (lang in row_json) {
          emailTemplateText = row_json[lang];
        ...
        let subject = "invoice list";

        email = "deepakkumar@zot.app";
...
const sendEmail = async (to_address, subject, emailTemplateText)=>{
    ...
    try {      
        let mailOptions = {
          ...
          var result = {};
          result = await transporter_credipay.sendMail(mailOptions)
    } catch (error) {
        console.log("errprrrrr====",error);
